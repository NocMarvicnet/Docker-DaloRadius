...
function userDisable($username, $divContainer) {
	...
	foreach ($username as $variable=>$value) {
        ...
        $sql1 = "INSERT INTO log_operadores (user_id, operator, change_description) VALUES ('$user','$operator',CONCAT(UPPER((SELECT firstname FROM operators WHERE username = '$operator')),' ',UPPER((SELECT lastname FROM operators WHERE username = '$operator')),' corto el usuario manualmente'))";
...
function userEnable($username, $divContainer) {
	...
	foreach ($username as $variable=>$value) {
		...
		if ($user) {
	        ...
	        $res = $dbSocket->query($sql);
            $sql1 = "INSERT INTO log_operadores (user_id, operator, change_description) VALUES ('$user','$operator',CONCAT(UPPER((SELECT firstname FROM operators WHERE username = '$operator')),' ',UPPER((SELECT lastname FROM operators WHERE username = '$operator')),' habilito el usuario manualmente'))";
            $res2= $dbSocket->query($sql1);

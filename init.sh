...
function init_database {
    ...
    echo "Importing custom SQL scripts from /custom-sql..."
    for file in /customsql/*.sql; do
    ...
    echo "Importing backup SQL scripts from /restore_backup..."
    for file in /restore_backup/*.sql; do
      echo "Importando $file..."
      if [ -f "$file" ]; then
        echo "Importando $file..."
        mysql -h "$MYSQL_HOST" -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" < "$file"
